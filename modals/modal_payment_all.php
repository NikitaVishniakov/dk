...
<form class="" method="post" action="#">
    ...
    <div id="transaction">
        ...
        <?php 
        $users = $link->query("SELECT id_referal FROM referals WHERE id_agent = '{$_SESSION['ID']}'"); 
...
?>
        ...
        <div class="form-group">                
            ...
            <input type="text" class="hidden" id="date" name="date" required="required" class="form-control" value="">
        </div>
        <div class="form-group">
            <button type="submit" id="transaction_save" name="submit" class="btn-success col-xs-12 col-md-12 col-sm-12 col-lg-12 btn">Создать платеж
                    ...
                    <script>$("#transaction_user").chosen({no_results_text: "пользователь не найден!"});</script></div>
        </div>
    </div>
<script>
